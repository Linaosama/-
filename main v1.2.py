...
# 函数定义区
def find_student(found_id, student):
    for stu in student:
        if stu["id"] == found_id:
            return stu
    return None
...
while True:
    ...
    if choice == "1":  # 添加学生
        ...
        if len(parts) != 2:
            ...
            if find_student(student_number, student) is not None:
                print("学生已存在，请重新输入！")
            else:
        ...
        found_student = find_student(student_num, student)
        ...
        target = find_student(found_id, student)
